...
def find_shortest_path_multiple(graph, start, items):
    ...
    for perm in permutations(items):
        ...
        for i in range(len(perm)):
            ...
            current_distance += sum(nx.dijkstra_path_length(graph, partial_path[j], partial_path[j+1]) for j in range(len(partial_path)-1))
...
def find_path():
    ...
    if 'Unknown' not in item_locations_list:
        ...
        plot_json = visualize_path_with_plotly(G, nx.get_node_attributes(G, 'pos'), shortest_path, f'Shortest Path to {item_names}')
        # Generate the text representation of the path
        path_text = ' -> '.join(shortest_path)
        return render_template('result.html', item_names=item_names, plot_json=plot_json, path_text=path_text)
    else:
